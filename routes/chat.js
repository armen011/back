...
router.post("/", async (req, res) => {
    try {
        ...
        const invitedRoom=await ChatRoom.find({creatorId:req.body.memberId,memberId:req.body.creatorId})
        ...
        if(myRoom.length>0 || invitedRoom.length>0){
            ...
            const chat={id:currentRoom[0].id,title:member.fullName,picture:member.profilePicture}
            res.status(200).json(chat)
        }else{
            const newRoom = new ChatRoom(req.body);
            const savedRoom = await newRoom.save();
            console.log("----",savedRoom.id)
            res.status(200).json({id:savedRoom.id,title:member.fullName,picture:member.profilePicture});
        }
	} catch (err) {
...
router.get("/", async (req, res) => {
	try {
		const myChats = await ChatRoom.find({creatorId:req.query.user_id}).then(async chats=>{
            if(chats.length===0){
                return chats
            }
            const result = await Promise.all(chats.map(async (chat) => {
                try{
                    ...
                    return {id:chat.id,title:user.fullName,picture:user.profilePicture}
                }catch(err){
                }
            }))
        ...
        const invitedChats = await ChatRoom.find({memberId:req.query.user_id}).then(async chats=>{
            if(chats.length===0){
                return chats
            }
            const result = await Promise.all(chats.map(async (chat) => {
...
router.get("/messages/:chatId/:userId", async (req, res) => {
	try {
		const resolvedMessages = await Message.find({chatId:req.params.chatId}).then(messages=>{
