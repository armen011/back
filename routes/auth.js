...
router.post("/check_user", async (req, res) => {
  ...
  if (!username || !email) {
    res
      .status(403)
      .json({ field: "", text: "Email and username are required" });
  } else if (isEmailUsed) {
    res.status(403).json({ field: "email", text: "Email is already used" });
  } else if (isUsernameUsed) {
    res
      .status(403)
      .json({ field: "username", text: "Username is already used" });
  } else {
  ...
  try {
    if (!username || !email || !password || !dateOfBirth || !fullName) {
      res
        ...
        .json({
          field: "",
          text: "email,username,password,dateOfBirth and fullName are required",
        });
    } else {
